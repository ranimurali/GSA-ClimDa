...
ifelse(tag>0,num_file<-num_file_pr,num_file<-num_file_bs)
 ...
 #    print(yrs_in_file)
     ...
     endyr <- 0
     pnt_ind <- 1
     ifelse(tag==0,startingyear<-Base_yr_s,startingyear<-Proj_yr_s)
     ifelse(tag==0,endingyear<-Base_yr_f,endingyear<-Proj_yr_f)
...
#-1/21       if ( ((countyear+k-1) >= Proj_yr_s) && ((countyear+k-1) <= Proj_yr_f) ) {
       if ( ((countyear+k-1) >= startingyear) && ((countyear+k-1) <= endingyear) ) {
 #        print("hit year")
 #        print(countyear+k-1)
...
#-1/21        if ((countyear+k-1) <= Proj_yr_f) {
        if ((countyear+k-1) <= endingyear) {
...
#  need to pull data into model matrix
  ...
  if (tag > 0) {
  fil_open<-c(paste(sep="","BCCAv2_0.125deg_pr_day_",Modeln[i],"_",Scen_n,"_r1i1p1_",pr_yr_s[j],"-",pr_yr_f[j],".nc"))}

  if (tag == 0) {
  fil_open<-c(paste(sep="","BCCAv2_0.125deg_pr_day_",Modeln[i],"_historical_r1i1p1_",bs_yr_s[j],"-",bs_yr_f[j],".nc"))}
