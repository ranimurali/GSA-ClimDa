...
bigtable[1,1]<-mean(dat1o$z,na.rm=TRUE)

dat1$z=ES_AC_ann_pr_tot
rrp=raster(dat1)
bigtable[1,4]<-mean(dat1o$z,na.rm=TRUE)

par(mfrow=c(2,2))  # two plots side-by-side (rows = 2, columns = 2)
#
plot(rr,main="Obs Annual Precip")

plot(rrp,main="Change in Ens Avg Annual Precip")
...
rr=raster(dat1)
bigtable[1,2]<-mean(dat1$z,na.rm=TRUE)

dat1$z=apply(Pr_ann_pr_tot,c(1,2),mean)
rr=raster(dat1)
bigtable[1,3]<-mean(dat1$z,na.rm=TRUE)

dat1$z=ES_PC_ann_pr_tot
rr=raster(dat1)
bigtable[1,5]<-mean(dat1$z,na.rm=TRUE)

dat1$z=apply(AC_ann_pr_tot,c(1,2),mean)
rr=raster(dat1)
bigtable[1,6]<-mean(dat1$z,na.rm=TRUE)-1.96*sd(dat1$z,na.rm=TRUE)/sqrt(length(dat1$z))
bigtable[1,7]<-mean(dat1$z,na.rm=TRUE)+1.96*sd(dat1$z,na.rm=TRUE)/sqrt(length(dat1$z))
...
rr=raster(dat1o)
bigtable[2,1]<-mean(dat1o$z,na.rm=TRUE)

par(mfrow=c(2,2))  # two plots side-by-side (rows = 1, columns = 2)
plot(rr,main="Obs Avg Daily Precip")

dat1$z=ES_AC_avg_daily_pr
rr=raster(dat1)
bigtable[2,4]<-mean(dat1$z,na.rm=TRUE)
plot(rr,main="Change in Ens Avg Daily Precip")
...
rr=raster(dat1)
bigtable[2,2]<-mean(dat1$z,na.rm=TRUE)

dat1$z<-apply(Pr_avg_daily_pr,c(1,2),mean)
rr=raster(dat1)
bigtable[2,3]<-mean(dat1$z,na.rm=TRUE)

dat1$z=ES_PC_avg_daily_pr
rr=raster(dat1)
bigtable[2,5]<-mean(dat1$z,na.rm=TRUE)

dat1$z=apply(AC_avg_daily_pr,c(1,2),mean)
rr=raster(dat1)
bigtable[2,6]<-mean(dat1$z,na.rm=TRUE)-1.96*sd(dat1$z,na.rm=TRUE)/sqrt(length(dat1$z))
bigtable[2,7]<-mean(dat1$z,na.rm=TRUE)+1.96*sd(dat1$z,na.rm=TRUE)/sqrt(length(dat1$z))
...
par(mfrow=c(2,2))  # two plots side-by-side (rows = 1, columns = 2)
plot(rr,main=c(paste("Obs Monthly Precip:",imtab,sep="")))

dat1$z=ES_AC_mon_avg_pr[,,imtab]
rr=raster(dat1)
bigtable[inmonth,4]<-mean(dat1$z,na.rm=TRUE)
plot(rr,main=c(paste("Change in Monthly Precip",imtab,sep="")))
...
plot(rr,main=c(paste("Obs Return Periods:",Ret_yrs[retyrs],sep="")))

dat1$z=ES_AC_retperiods[,,retyrs]
rr=raster(dat1)
bigtable[cntyrs,4]<-mean(dat1$z,na.rm=TRUE)
plot(rr,main=c(paste("Change in Return Periods:",Ret_yrs[retyrs],sep="")))
...
plot(rr,main=c(paste("Obs Days below threshold:",Thres_val[ithres],sep="")))

dat1$z=ES_AC_threvalue[,,ithres]
rr=raster(dat1)
bigtable[cntloc,4]<-mean(dat1$z,na.rm=TRUE)
plot(rr,main=c(paste("Proj Days below threshold:",Thres_val[ithres],sep="")))
...
cntloc = cntloc + 1
bigtable[cntloc,1]<-mean(dat1o$z,na.rm=TRUE)
plot(rr,main=c(paste("Obs Days above threshold:",Thres_val[ithres],sep="")))

dat1$z=ES_AC_threvalue[,,ithres+1]
rr=raster(dat1)
bigtable[cntloc,4]<-mean(dat1$z,na.rm=TRUE)

plot(rr,main=c(paste("Proj Days above threshold:",Thres_val[ithres],sep="")))
...
#rr=raster(dat1)
#bigtable[cntloc,5]<-mean(rr,na.rm=TRUE))
bigtable[cntloc,5]<-"n/a"
...
if (thresprecip !=1) {cntloc = Num_thre + 1+ 14}
...
rr=raster(dat1o)
bigtable[cntloc,1]<-mean(dat1o$z,na.rm=TRUE)
plot(rr,main=c(paste("Obs Maximum 5 Day Precip")))

dat1$z=ES_AC_5day
rr=raster(dat1)
bigtable[cntloc,4]<-mean(dat1$z,na.rm=TRUE)

plot(rr,main=c(paste("Proj Change in Maximum 5 Day Precip")))
...
rr=raster(dat1o)
bigtable[cntloc,1]<-mean(dat1o$z,na.rm=TRUE)
plot(rr,main=c(paste("Obs Maximum Consecutive Dry Days Per Year")))

dat1$z=ES_AC_consdd
rr=raster(dat1)
bigtable[cntloc,4]<-mean(dat1$z,na.rm=TRUE)

plot(rr,main=c(paste("Proj Change in Cons Dry Days Per Year")))
...
rr=raster(dat1o)
bigtable[cntloc,1]<-mean(dat1o$z,na.rm=TRUE)
plot(rr,main=c(paste("Obs Simply Daily Intensity Index")))

dat1$z=ES_AC_sdii
rr=raster(dat1)
bigtable[cntloc,4]<-mean(dat1$z,na.rm=TRUE)

plot(rr,main=c(paste("Proj Change in Simple Daily Intensity Index")))
