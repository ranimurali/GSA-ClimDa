...
#MODIFIED TO calculate Tmax Tmin custom range (same as new function Trange (but signs reversed))
#-----
      ...
      for (jj in 1:iend){ 
        ...
        if (tag==1) {
          if ( (sum(is.na(Pr_Tmin_val[lonlon,latlat,jj,i]))==0) &&
            ...
            if ((Pr_Tmin_val[lonlon,latlat,jj,i]>trigMin)&&(Pr_Tmax_val[lonlon,latlat,jj,i]<trigMax)) {
        ...
        if (tag==0) {
          if ( (sum(is.na(Tmin_val[lonlon,latlat,jj,i]))==0) &&
            ...
            if ((Tmin_val[lonlon,latlat,jj,i]>trigMin)&&(Tmax_val[lonlon,latlat,jj,i]<trigMax)) {
              fredum[lonlon,latlat]<-fredum[lonlon,latlat] + 1}}
        } # if Tag 0
      }   #jj
    } # latlat
    ...
    for (latlat in 1:ob_lat_num) {
      for (jj in 1:iend){
        if (  (sum(is.na(Ob_tmin[lonlon,latlat,jj]))==0) &&
          ...
          if ((Ob_tmin[lonlon,latlat,jj]>trigMin)&&(Ob_tmax[lonlon,latlat,jj]<trigMax)) {
            fredum[lonlon,latlat]<-fredum[lonlon,latlat] + 1}
        } # if number
      }   # jj
    } # latlat
