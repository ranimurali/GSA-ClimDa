...
#Resolution:            1 degree
#Latitude bounds:       (25.0, 53.0)<- values are centered at 25.5 to 52.5 N ; 28 lat
#Longitude bounds:      (-125.0, -67.0) <-- values are centered at -124.5 to -67.5E ; 58 lon
...
if (temp=="Tmin") {
fil_dir<-c(paste(sep="",ClimDAloc,"\\US\\obs\\1_deg\\tmin"))}
if (temp=="Tmax") {
fil_dir<-c(paste(sep="",ClimDAloc,"\\US\\obs\\1_deg\\tmax"))}
...
if (temp=="Tmin") {fil_open<-c("Extraction_tasmin.nc")}
if (temp=="Tmax"){fil_open<-c("Extraction_tasmax.nc")}

  print("opening file:")
  print(fil_open)

  tm_2 = nc_open(fil_open) 
if (temp=="Tmin") {tm_val_whole <-ncvar_get(tm_2,"tasmin") }
if (temp=="Tmax"){tm_val_whole <-ncvar_get(tm_2,"tasmax") }
  
  Ob_tp_val <-tm_val_whole[,,startyr:endyr]
