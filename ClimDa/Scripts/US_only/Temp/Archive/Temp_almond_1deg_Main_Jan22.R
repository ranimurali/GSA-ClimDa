...
#source('Temp_almond_1deg_Main.R')
...
Ob_almon6<-Ob_dum/tot_yrs

almonT = 7
source('almond_temp_indicators.R')
Ob_almon7<-Ob_dum3_t/tot_yrs

almonT =8
source('almond_temp_indicators.R')
Ob_almon8<-Ob_dum/tot_yrs
...
#---------Tavg
Temp_val<- (Tmin_val[,,,i]+Tmax_val[,,,i])/2  #Ob_tavg<-(Ob_tmax+Ob_tmin)/2
...
rr=raster(dat1o)
bigtable[cntyrs,1]<-mean(extract(rr,reg_cutout,weights=TRUE,fun=mean,na.rm=TRUE))
plot(crop(rr,reg_cutout),main=c(paste("Blooms Days Tmax (F) > < ",Al7_Thres_val[ithres],sep="")))
plot(reg_cutout,add=TRUE)
