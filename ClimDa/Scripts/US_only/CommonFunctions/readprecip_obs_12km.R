...
# directory obs file is located
 ...
 latcount<-ob_lat_num
   ...
   for (k in 1:bs_yr_num) {
       ...
       if (Module_ch=="Temp" & temp =="Tmax") {
         fil_open<-c(paste(sep="",fil_dir,"gridded_obs.tasmax.OBS_125deg.daily.",yr_file,".nc"))}
       if (Module_ch=="Temp" & temp =="Tmin") {
         fil_open<-c(paste(sep="",fil_dir,"gridded_obs.tasmin.OBS_125deg.daily.",yr_file,".nc"))}
...
# take into account leap years   
       ...
       if (Module_ch=="Temp") {
         temp_2 = nc_open(fil_open)
       if (temp=="Tmax") { 
         Ob_tp_val[,,startday:endday] <-ncvar_get(temp_2,"tasmax",start=c(lon1_ind,lat1_ind,1),count=c(loncount,latcount,daysinyr)) }
       if (temp=="Tmin") {
         Ob_tp_val[,,startday:endday] <-ncvar_get(temp_2,"tasmin",start=c(lon1_ind,lat1_ind,1),count=c(loncount,latcount,daysinyr)) }
       }   # module_ch - temp
...
#  setwd("F:\\ClimDA\\Scripts\\US_only\\Precip\\Functions")
  if (Module_ch=="Precip") {remove(pr_2)}
  if (Module_ch=="Temp") {remove(temp)}